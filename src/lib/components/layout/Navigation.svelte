			...
			<div class="hidden lg:flex items-center space-x-8">
				{#each navLinks as link}
					<a
						...
						class="transition-colors duration-200 font-medium relative group {$page.url.pathname === link.href ? 'text-brand-primary' : 'text-text-secondary hover:text-brand-primary'}"
					>
						{link.label}
						<span
							class="absolute -bottom-1 left-0 h-0.5 bg-brand-primary transition-all duration-300 {$page.url.pathname === link.href ? 'w-full' : 'w-0 group-hover:w-full'}"
						></span>
					</a>
				{/each}
